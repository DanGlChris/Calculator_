        ...
        for(int i = entry.length-1; i >=0 ; i--) {            
            if(entry[i].isUnity()){
                if(i-1>=0 && entry[i].ordinal()>1 && entry[i-1].ordinal() == (entry[i].ordinal()-2)){   //U(n-2)--U(n) (IX:9, XC:90, CM:900)
                    ...
                    value+= temp_unity;
                    if(temp_unity<min_unity) System.out.println("Erreur 1"); //throw new Exception("Не правилно вход");
                    ...
                    System.out.println("Erreur 2");//throw new Exception("Не правилно вход");
                    ...
                    value+= temp_unity;
                    if(temp_unity<min_unity || temp_unity%min_unity!=0) System.out.println("Erreur 3");// throw new Exception("Не правилно вход");
                    ...
                    tour =1;
                    System.out.println("Je suis passee,  " + entry[i].name() + " -- " + i);
                    while(entry[i-1].ordinal() == entry[i].ordinal()){
                        tour++;
                        if(tour>3){ System.out.println("Erreur 4"); break;}// throw new Exception("Не правилно вход"); 
                        else if(i==1) {
                            i=i-1; break;
                        }
                        else i=i-1;
                    ...
                    System.out.println("Je suis passee, touor " + i);
                    value+=temp_unity;
                    if(temp_unity<min_unity) System.out.println("Erreur 5"); //throw new Exception("Не правилно вход");
                    else min_unity = 5*Roman_Numeral_Sign.values()[entry[i+1].ordinal()].getvalue();
                }else if(i==0){
                    temp_unity = entry[i].getvalue();
                    value+= temp_unity;
                    if(temp_unity<min_unity) System.out.println("Erreur 6"); //throw new Exception("Не правилно вход");
                ...
                if(i-1>=0 && entry[i-1].ordinal() == (entry[i].ordinal()-1)){                    
                    temp_unity=entry[i].getvalue()-entry[i-1].getvalue();
                    value+= temp_unity;
                    if(temp_unity <min_unity) System.out.println("Erreur 7"); //throw new Exception("Не правилно вход");
                    else min_unity = entry[i].getvalue()*2;
                    i--;
                }else if(i-1>=0 && entry[i-1].ordinal() != (entry[i].ordinal()-1)){
                    temp_unity = entry[i].getvalue();
                    value+= temp_unity;
                    if(temp_unity<min_unity) System.out.println("Erreur 8"); //throw new Exception("Не правилно вход");
                    else min_unity =  entry[i].getvalue()*2;;                    
                }else if(i==0){
                    temp_unity = entry[i].getvalue();
                    value+= temp_unity;
                    if(temp_unity<min_unity) System.out.println("Erreur 9"); //throw new Exception("Не правилно вход");
